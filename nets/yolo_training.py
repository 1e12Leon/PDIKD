...
# from utils.loss import LIKDLoss
...
class YOLOLoss(nn.Module):
    ...
    def comupte_feat_distill_loss(self, feats, t_feats):
        featLoss = nn.MSELoss(reduction="none")
        # featLoss = LIKDLoss()
        feat_loss = 0
        for i, feat in enumerate(feats):
            t_feat = t_feats[i]
            feat_loss += torch.mean(featLoss(feat, t_feat))
        return feat_loss
