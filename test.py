...
if __name__ == "__main__":
    ...
    #   默认指向根目录下的VOC数据集
    # -------------------------------------------------------#
    VOCdevkit_path = 'H:\ozhiquan\yolov5-pytorch\VOCdevkit'
    # -------------------------------------------------------#
    #   结果输出的文件夹，默认为map_out
    # -------------------------------------------------------#
    map_out_path = 'map_out_fft2'

    image_ids = open(os.path.join(VOCdevkit_path, "test/test.txt")).read().strip().split()
    ...
    if map_mode == 0 or map_mode == 1:
        ...
        for image_id in tqdm(image_ids):
            image_path = os.path.join(VOCdevkit_path, "test/JPEGImages/" + image_id + ".jpg")
    ...
    if map_mode == 0 or map_mode == 2:
        ...
        for image_id in tqdm(image_ids):
            with open(os.path.join(map_out_path, "ground-truth/" + image_id + ".txt"), "w") as new_f:
                root = ET.parse(os.path.join(VOCdevkit_path, "test/Annotations/" + image_id + ".xml")).getroot()
